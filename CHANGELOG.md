...
- Make webhook recording idempotent per Postal delivery. The row id is derived from the envelope `uuid`, so Postal's retries hit the primary key instead of creating rows, and `postal.<status>` is emitted once, through `emitEventStep`, only for a newly inserted row. The model's unique `(message_id, event_type)` index was never created by any migration and would have discarded legitimate repeated events; it is removed from the model. No migration is needed.

...
- Record real Postal webhooks. Postal wraps every callback as `{ event, timestamp, payload, uuid }`, but the plugin looked for the message tag only at the top level, so every genuine callback was acknowledged with `202 ignored` and nothing was stored or emitted. The envelope is now unwrapped (bare event hashes still work), `SoftFail`/`HardFail` map to `delayed`/`failed`, Postal's numeric message ids and float epoch timestamps are read, and the full body is stored. **Behaviour change:** after upgrading, webhook rows start appearing and `postal.<status>` subscribers start firing on real traffic for the first time; every open and click is a row.
- Make webhook recording idempotent per Postal delivery. The row id is derived from the envelope `uuid`, so Postal's retries hit the primary key instead of creating rows, and `postal.<status>` is emitted once, through `emitEventStep`, only for a newly inserted row. If the emit fails, the row is removed again so Postal's retry records and emits it. The model's unique `(message_id, event_type)` index was never created by any migration and would have discarded legitimate repeated events; it is removed from the model. No migration is needed.
- Bound the whole Postal API exchange by `POSTAL_REQUEST_TIMEOUT_MS`. The timer covered only the response headers, so a Postal server that sent headers and then stalled the body hung the send indefinitely; it also started before the config lookup and leaked when that lookup failed. The config is now resolved once per send.
- Require an http(s) `base_url` wherever it comes from. Only provider options were checked, so a value from `POSTAL_BASE_URL` or saved in the admin was used unchecked; the admin settings schema now rejects it and the provider refuses it before any request.
- Send `cc`/`bcc` once. A send to several `to` recipients becomes one Postal send per recipient, and every one of them carried the same `cc`/`bcc`, so each copy recipient got one email per `to` recipient. The copies now ride on the first send only.
...
- Accept any Medusa 2.x from 2.19.0. The Medusa peers were pinned to exactly `2.19.0` (the upgrade to 2.20.1 changed only devDependencies), so npm refused to install the plugin next to Medusa 2.20.x/2.21.x with `ERESOLVE`. Peers are now `^2.19.0` (`@medusajs/ui ^4.2.1`), `@tanstack/react-query` is aligned with the `5.64.2` the dashboard ships, and the unused `zod` peer is dropped. Medusa 2.18.0 hosts should stay on 0.3.7 or earlier; 0.3.8 through 0.4.2 require exactly Medusa 2.19.0.
- The admin Postal page can save settings and send test emails again. Both requests were rejected by the strict schemas before reaching the server: the save sent the display-only `api_key` field (secret fields are rejected by design), and the test send posted `action: "test"` to `/admin/postal/send-test`, whose schema has no `action`. The page now sends only the non-secret settings, and `/admin/postal/send-test` accepts an empty `to` and `subject`, falling back to the saved test recipient and the template subject as the workflow already did.

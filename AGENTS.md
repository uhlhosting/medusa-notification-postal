...
8. Postal webhook callbacks must use the tokenized public route `POST /postal/webhooks/:token` (outside `/store`, so no publishable API key is required), and the exact tokenized URL should be surfaced from an admin-only view rather than the settings surface
...
15. The webhook route accepts Postal's `{ event, timestamp, payload, uuid }` envelope (and a bare event hash for backward compatibility). Recording is idempotent per Postal delivery: the row id is `postal_webhook_<uuid>`, which stays the same across Postal's retries, so the primary key rejects replays. There is deliberately no unique `message_id` + `event_type` constraint — Postal repeats `MessageLoaded`/`MessageLinkClicked`/`MessageDelayed` for one message, and each is a distinct event. A replay is detected by re-reading the row by id, never by inspecting the database error, because Medusa maps unique violations to a `MedusaError` without the pg code. `postal.<status>` is emitted through `emitEventStep` at most once, only when a row was newly inserted, and released when the workflow succeeds; if the emit step fails, the record step's compensation hard-deletes the inserted row (a soft delete would keep the primary key and turn every retry into an error) so Postal's retry records and emits it. All recognized lifecycle events are persisted without swallowing generic database errors.

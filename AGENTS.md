...
10. `@medusajs/*` packuments carry thousands of preview/snapshot versions, so npmjs rate-limits metadata requests for them (`medusajs/medusa#16294`). pnpm's supply-chain verification re-applies `minimumReleaseAge` / `trustPolicy` to all ~1600 lockfile entries on **every** install, which reliably trips that limit and kills the job with `ERR_PNPM_FETCH_429` before any step runs. Both CI systems therefore install with `--trust-lockfile`: GitLab via `PNPM_INSTALL_FLAGS` (the `validate` job overrides it to `""` so the lockfile is still verified against the policy exactly once per pipeline), and `.github/workflows/npm-publish.yml` directly. Keep the pnpm version in `.gitlab-ci.yml` (`PNPM_VERSION`) and in `npm-publish.yml` (`corepack prepare`) in sync with the `packageManager` field in `package.json`. Never add npm auth to raise the rate limit — invariant: no npm tokens in this repo.

...
20. The admin extension is bundled into `@medusajs/dashboard` and must share its `@medusajs/ui`, `@tanstack/react-query`, `i18next`, `react-i18next` and `react-router-dom` instances; Medusa's admin bundler does not dedupe them, so a second installed version is bundled as a second copy and the plugin pages crash on npm hosts. Their `peerDependencies` **and** `devDependencies` are pinned to the exact versions the targeted dashboard depends on (for 2.20.1: `4.2.3`, `5.64.2`, `23.7.11`, `13.5.0`, `7.18.2`) — never widened to ranges. The `@medusajs/*` peers equal their devDependencies. Re-align all of them in the same commit as a Medusa upgrade (`npm view @medusajs/dashboard@<version> dependencies`); `scripts/verify-release.mjs` fails on drift.
21. `typecheck:admin` must type-check translation keys against the real dashboard `Resources`: `@medusajs/dashboard` is a devDependency (pnpm 11 ignores `.npmrc` hoist settings, so it never resolves transitively), and the `i18next` `CustomTypeOptions` augmentation lives in `src/admin/i18next.ts` — not a `.d.ts`, which `skipLibCheck` skips, silently turning `Resources` into `any` when the import breaks.
...
1. `pnpm release:check` passes (includes admin typecheck via `typecheck:admin` and the Medusa/dashboard peer-alignment check in `verify-release.mjs`)

...
10. CI installs run `pnpm install --frozen-lockfile --prefer-offline $PNPM_INSTALL_FLAGS`. `PNPM_INSTALL_FLAGS` defaults to `--trust-lockfile`; the `validate` job overrides it to `""` so the lockfile is verified against pnpm's supply-chain policy (`minimumReleaseAge` / `trustPolicy`) **exactly once per pipeline**. Re-verifying in every job means a full registry metadata sweep of ~1600 packages per job, which npmjs rate-limits (`ERR_PNPM_FETCH_429`) and which fails the pipeline before any test runs. Keep `PNPM_VERSION` in `.gitlab-ci.yml` in sync with the `packageManager` field in `package.json`. Never add npm auth to raise the rate limit — invariant: no npm tokens in this repo.
